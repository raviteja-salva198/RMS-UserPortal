...
const PremiumPlans = () => {
  ...
  const token = Cookies.get('token')  //"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImpvaG5kb2UiLCJpZCI6IjEyMzQ1NiIsImlhdCI6MTYwNTgyMjAwMCwiZXhwIjoxNjA1ODI1NjAwfQ.xRpzKvFXP4_eB4z0dUkFjckYvMSEU1RVAV2YseEpTlk"
  ...
  useEffect(() => {
    const fetchCurrentPlan = async () => {
      try {
        const response = await axios.get(`${API_URL}/current-plan/${userId}`);
  ...
  const handleBuyPlan = async (planName) => {
    try {
      await axios.post(`${API_URL}/buy-plan/${userId}`, { planName, email: "ravitejasalva@gmail.com" });
